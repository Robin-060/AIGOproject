# OBS Trust Layer 开放探索日志（EXP01–17 最终版）

> 用途：复赛可检查的完整探索记录。每条按
> Hypothesis → Experiment → Observation → Revision → Result → Limitation 组织，
> 所有定量表述均绑定产物；历史口径如被后续审计修正，原文留档并显式标注替代关系。
> 覆盖：项目全程（含被推翻的早期方法），17 个实验条目 + 负结果清单 + 数字速查表。
...
- 历史数字（29.1%→2.8%、5.5%@50 等）**不得引用**，已由 v1.5.1-bugfix 全链重跑取代
